...
//
// Key storage: the app's main preferences store (preferences.js) uses a strict
// electron-store schema we must not modify, so the Claude API key lives in its
// own small store here. The renderer reads/writes it via the ai-get-key /
// ai-set-key IPC channels.
//
...
// Dedicated store for the BYO key — separate from the strict-schema app prefs.
const keyStore = new Store({ name: 'ai-credentials' })

export function getClaudeApiKey() {
  // Prefer an explicitly stored key; also honor an env var for power users.
  return keyStore.get('claudeApiKey') || process.env.ANTHROPIC_API_KEY || ''
}

export function setClaudeApiKey(key) {
  if (typeof key === 'string' && key.trim()) {
    keyStore.set('claudeApiKey', key.trim())
  } else {
    keyStore.delete('claudeApiKey')
  }
  return { ok: true }
}
